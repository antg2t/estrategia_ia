...
(async()=>{
 ...
 await page.locator('#btn-next').click();check(await page.locator('#trabalho-transicao').isVisible(),'Work transition must follow agenda');
 await page.locator('#btn-next').click();check(await page.locator('#trabalho').isVisible(),'Work follows its transition');
 ...
 await page.locator('#mercado [data-jump="nubank-transicao"]').click();
 check(await page.locator('#nubank-transicao').isVisible(),'Case transition opens');
 check(await page.locator('#return-strip').isVisible(),'Drill return shown');
 await page.locator('#btn-next').click();check(await page.locator('#nubank').isVisible(),'Next enters the case');
 await page.keyboard.press('ArrowRight');check(await page.locator('#performance').isVisible(),'Arrow continues within case');
 await page.keyboard.press('ArrowLeft');check(await page.locator('#nubank').isVisible(),'Previous stays in case');
 ...
 const groups=['Percurso','Trabalho','Mercado','Vivo','Direção','Execução'];
 check(JSON.stringify(await page.evaluate(()=>[...new Set(DATA.slides.map(s=>s.group))]))===JSON.stringify(groups),'Six consistent narrative groups');
 await page.locator('#btn-help').click();
 check(!/assessment|duração|\b(?:horas|minutos)\b/i.test(await page.locator('#dialog-body').innerText()),'Help has no removed content');
 await page.keyboard.press('Escape');
 await page.locator('#btn-map').click();
 check(JSON.stringify(await page.locator('.map-group h3').allTextContents())===JSON.stringify(groups),'Map follows agenda');
 await page.keyboard.press('Escape');
 const html=fs.readFileSync(path.join(root,'outputs/segunda-feira/REUNIAO_ESTRATEGIA_IA_VP_PESSOAS.html'),'utf8');
 check(!/assess?ement|assessment|SRC-037|Escolha a duração|\b(?:20 minutos|2 horas|4 horas)\b/i.test(html),'Removed material absent from complete HTML');
 check(await page.locator('[data-mode]').count()===2,'Exactly two modes');
 for(const mode of ['executivo','estendido']){
   await page.locator(`[data-mode="${mode}"]`).click();
   const route=await page.evaluate(()=>({ids:route()}));
   report.routes[mode]={count:route.ids.length};
   ...
   for(let i=0;i<route.ids.length;i++){
     ...
     check(await page.locator('#slide-position').innerText()===`${Math.round((i+1)/route.ids.length*100)}%`,'Progress follows slide position');
 ...
 for(const [i,s] of slides.entries()){
   await page.goto(file+`#modo=estendido&quadro=${s.id}`);
 ...
 for(const id of ['abertura','apostas','camadas','comparacao']){
   await page.goto(file+`#modo=estendido&quadro=${id}`);
   await page.screenshot({path:path.join(out,`mobile-${id}.png`),fullPage:true});
 }
 await page.goto(file+'#modo=executivo&quadro=mercado');
 await page.locator('#mercado [data-jump="nubank-transicao"]').click();await page.locator('#btn-next').click();await page.reload();
 ...
 await page.goto(file+'#modo=estendido&quadro=comparacao');
 ...
 await page.goto(file+'#modo=estendido&quadro=camadas');await page.locator('[data-layer="2"]').click();
 ...
 await page.goto(file+'#modo=executivo&quadro=trabalho');await page.context().setOffline(true);
 ...
 const printVisible=await page.locator('section.vpp-slide:visible').count();check(printVisible===report.routes.executivo.count,'Print follows selected route');
 ...
 await browser.close();
 check(!report.desktop.some(x=>x.overflow.length||x.overlapsSources),'Desktop slides must fit');
 check(!report.mobile.some(x=>x.overflow),'Mobile slides must fit');
