...
(async()=>{
 ...
 const report=await page.evaluate(()=>{
  ...
  const issues=[];
  for(const id of ['trabalho-transicao','mercado','vivo-transicao','direcao-transicao','apostas','execucao-mapa']){
   ...
   for(const button of buttons){
    ...
    const heading=plain(document.querySelector('#'+dest+' .screen-title'));
    if(title!==heading)issues.push(`${id}: ${title} != ${heading}`);
   }
   for(const [mode,route] of Object.entries(DATA.routes)){
    if(!['apostas','execucao-mapa'].includes(id))continue;
